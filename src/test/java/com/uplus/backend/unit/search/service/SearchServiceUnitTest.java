...
public class SearchServiceUnitTest {
    ...
    void 검색_연관검색어_조회_테스트() {
        // given
        given(deviceRepository.findTop5ByNameContainingIgnoreCaseAndNetworkTypeOrderByLaunchedDateDesc("갤럭시", 5)).willReturn(List.of(device1));
        given(deviceRepository.findTop5ByNameContainingIgnoreCaseOrderByLaunchedDateDesc("갤럭시")).willReturn(List.of(device1));

        // when
        SearchKeywordListResponseDto responseDtoWithNoNetworkType = searchService.getSearchKeyword("갤럭시", 0);
        SearchKeywordListResponseDto responseDtoWithNetworkType = searchService.getSearchKeyword("갤럭시", 5);

        // then
        assertThat(responseDtoWithNoNetworkType.getSearchKeywordList().get(0).getName()).isEqualTo(device1.getName());
        assertThat(responseDtoWithNetworkType.getSearchKeywordList().get(0).getName()).isEqualTo(device1.getName());
        verify(deviceRepository).findTop5ByNameContainingIgnoreCaseAndNetworkTypeOrderByLaunchedDateDesc(anyString(), anyInt());
        verify(deviceRepository).findTop5ByNameContainingIgnoreCaseOrderByLaunchedDateDesc(anyString());
    }

    @Test
    void 검색_리스트_조회_테스트() {
        // given
        given(deviceRepository.findByNameContainingIgnoreCaseAndNetworkTypeOrderByLaunchedDateDesc("갤럭시", 5)).willReturn(List.of(device1));
        given(deviceRepository.findByNameContainingIgnoreCaseOrderByLaunchedDateDesc("갤럭시")).willReturn(List.of(device1));

        // when
        SearchListResponseDto responseDtoWithNoNetworkType = searchService.searchByKeyword("갤럭시", 0);
        SearchListResponseDto responseDtoWithNetworkType = searchService.searchByKeyword("갤럭시", 5);

        // then
        assertThat(responseDtoWithNoNetworkType.getSearchList().get(0).getName()).isEqualTo(device1.getName());
        assertThat(responseDtoWithNetworkType.getSearchList().get(0).getName()).isEqualTo(device1.getName());
        verify(deviceRepository).findByNameContainingIgnoreCaseAndNetworkTypeOrderByLaunchedDateDesc(anyString(), anyInt());
