...
public class SearchControllerUnitTest {
	...
	void 검색_연관검색어_조회_테스트() throws Exception {
		...
		SearchKeywordListResponseDto responseDto = SearchKeywordListResponseDto.fromEntity(List.of(device1));

		String query = "갤럭시";

		given(searchService.getSearchKeyword(query, device1.getNetworkType())).willReturn(responseDto);
	...
	void 검색_리스트_조회_테스트() throws Exception {
		...
		given(searchService.searchByKeyword(query, device1.getNetworkType())).willReturn(responseDto);
